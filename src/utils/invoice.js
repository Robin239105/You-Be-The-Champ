export const downloadInvoice = (orderData) => {
  // Normalize order data to handle both Checkout state and API response
  const order = {
    orderId: orderData.orderId || orderData.id,
    date: orderData.date || (orderData.createdAt ? new Date(orderData.createdAt).toLocaleDateString() : new Date().toLocaleDateString()),
    formData: orderData.formData || {
      firstName: orderData.user?.name?.split(' ')[0] || 'Customer',
      lastName: orderData.user?.name?.split(' ')[1] || '',
    ...
    items: orderData.items || (orderData.orderItems?.map(oi => ({
  ...
  const invoiceContent = `
    ...
    <head>
      ...
      <style>
        body { font-family: 'Arial', sans-serif; color: #1a1a1a; padding: 40px; line-height: 1.6; background: #fff; }
        ...
        .logo-img { width: 60px; height: 60px; object-fit: cover; border-radius: 50%; border: 2px solid #c9a84c; }
        ...
        .grand-total { font-size: 20px; font-weight: bold; color: #c9a84c; margin-top: 15px; border-top: 2px solid #c9a84c; padding-top: 10px; }
